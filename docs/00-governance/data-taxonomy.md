...
| Analytics | GA4 franchise Canada `259747921` / `G-6EMKPZD7RD` | Franchise Canada event analytics | GA4 event and parameter IDs | GA4 event names | Receives mapped `generate_lead`; not CRM truth | `analytics_259747921.events_*` after first export | `Active guarded`; native daily link verified 2026-07-31, dataset pending | [native export activation](../20-bigquery/franchise-ga4-native-bigquery-export-activation-2026-07-31.md) |
| Analytics | GA4 franchise USA `519783092` / `G-YL1KQPWV0M` | Franchise USA event analytics | GA4 event and parameter IDs | GA4 event names | Receives helper-submit reporting; not CRM truth | `analytics_519783092.events_*` after first export | `Active guarded`; native daily link verified 2026-07-31, dataset pending | [native export activation](../20-bigquery/franchise-ga4-native-bigquery-export-activation-2026-07-31.md) |
...
| Consolidated business data | BI-owned Supabase | Reported Parent business outcomes and BI serving | source IDs, `cefa_event_id`, Form entry ID, opportunity/enrollment IDs where verified | lifecycle labels, original timestamps, corrections and freshness | Parent consolidated business-data layer; exact contract pending | Power BI/Lovable and future minimum BigQuery outcome feed | `Partial`; read-only schema/pipeline verification pending | [alignment decision](../70-growth-operations/marketing-bi-alignment-final-decision-and-email-2026-07-27.md) |
...
| Warehouse | BigQuery project `marketing-api-488017` | Raw, staging, mart, dashboard, rule-registry and aggregate operational-health surfaces | table/view keys by source; `school_uuid`; platform IDs | mart field names, rule rows, fixed health metric keys | Reconciles source evidence; does not replace form/CRM truth | `analytics_267558140`, `analytics_259747921`, `analytics_519783092`; `raw_*`, `mart_*`, `cefa_core`; `cefa_ops.vw_measurement_activation_health_v1` | `Active guarded`; franchise native exports live, Canada nonproduction-host filtering and health runtime instrumentation pending | [operational health V1](../20-bigquery/measurement-activation-operational-health-scorecard-v1-2026-08-04.md), [native export activation](../20-bigquery/franchise-ga4-native-bigquery-export-activation-2026-07-31.md), [warehouse current state](../20-bigquery/warehouse-current-state-2026-05-03.md) |
...
| BI Supabase Parent outcome contract | `Pending` | BI and marketing data owners | Verify grain, identity, history, lineage, timestamps, freshness and proof-of-concept record before production marketing use. |
