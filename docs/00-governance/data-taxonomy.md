...
| Server-side tagging | Stape Business | Parent first-party production route plus planned isolated Franchise Canada and Franchise USA containers | `cefa_event_id`, hostname/site context, destination IDs | neutral event names and governed parameters | Additive server transport with exact browser/server deduplication | GA4, Google Ads, Meta and BigQuery diagnostics | `Active guarded`; Parent web version `15` and server version `13` have GA4, Meta CAPI, Google Conversion Linker and Google Ads Inquiry Submit live with shared Form 4 identity; franchise routes remain separate and pending | [Parent sGTM completion](../10-conversion-tracking/parent-sgtm-completion-2026-08-05.md) |
...
| Warehouse | BigQuery project `marketing-api-488017` | Raw, staging, mart, dashboard, rule-registry surfaces | table/view keys by source; `school_uuid`; platform IDs | mart field names, rule rows | Reconciles source evidence; does not replace form/CRM truth | `analytics_267558140`, `raw_*`, `mart_*`, `cefa_core` | `Active guarded`; Parent totals/schools usable, current paid-source allocation blocked pending evidence contract v2; predictive surfaces remain heuristic | [Parent omnichannel roadmap](./parent-omnichannel-measurement-and-intelligence-roadmap-2026-08-08.md) |
...
| Stape Parent routing, parity and rollback | `Active guarded` | Measurement engineering | Parent web version `15` and server version `13` route GA4, Meta and Google through the first-party endpoint. Controlled shared-identity QA passed; operational destination diagnostics, recovery custody and rollback ownership continue. |
| Parent paid-source classification | `Active guarded` | BigQuery/data | Total inquiries and selected schools remain usable, but current paid-channel allocation is blocked until evidence contract v2, backfill and assertions pass. |
| Predictive and MMM promotion | `Pending` | Marketing intelligence | Existing forecasts, creative labels, fatigue scores and MMM readiness are heuristic candidates; no registered BigQuery model exists. |
