...
| Stape Parent routing, parity and rollback | `Active guarded` | Measurement engineering | GA4 first-party routing is live; Google browser Transaction ID and the shared server identity contract are live; paused server destination tags are staged. Natural-event parity, controlled Google/Meta deduplication, Meta credential, consent-state operations and final rollback signoff remain pending. |
