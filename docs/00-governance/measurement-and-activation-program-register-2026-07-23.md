...
| Stape Business sGTM | `Active guarded` | On 2026-07-28 CEFA created Parent server GTM `GTM-T7C46VM7` and the matching Stape container in `CA East (Canada)`. Annual Business is paid and verified with `5M` requests/month, 10-day logs, 20-domain capability, and healthy billing. Parent web GTM `GTM-NZ6N7WNC` retains zero unpublished changes or conflicts and its live Parent GA4 Google tag has no `server_container_url` or transport override. In isolated server workspace `CEFA \| Parent \| sGTM Shadow Build \| 2026-07-28` (`3`), GTM MCP added exact-match trigger `CEFA \| Parent \| Shadow Health \| Event` for only `cefa_sgtm_shadow_health` and native server-GA4 tag `CEFA \| Parent \| GA4 \| Shadow Health`; read-back showed two pending changes, zero conflicts, no version and no publication. On 2026-07-29 the required Stape Canada CNAMEs, `Ready / Ok`, TLS and `/healthy` all passed. A PII-free `/g/collect` health hit returned HTTP `200` and the inherited GA4 client issued first-party `FPID`, proving endpoint/client handling without a published forwarding tag. Browser routing remains inactive, reported usage remains zero, and all optional power-ups remain off | Open server Preview, capture the temporary preview header, prove the draft trigger/tag with one synthetic hit, remove the temporary header, rotate the disclosed API key, and complete account-control review before any production routing |
...
| Parent sGTM draft preview session | CEFA measurement owner | The GTM MCP/API can create and verify the isolated workspace but does not expose Google's dynamic `X-Gtm-Server-Preview` value. The correct CEFA server workspace is open in Chrome with the two pending changes visible | Click `Preview`, copy the temporary manual-request header, run the PII-free `cefa_sgtm_shadow_health` proof through `edge.cefa.ca`, verify client/trigger/tag/outgoing request, then remove or let the temporary preview-header power-up expire |
...
| Track B - website | Buy Parent Business and verify Stape/API/GTM control planes, domains and routing architecture | `Active setup`; Business, Parent control planes and first-party endpoint health verified | CEFA ownership, recovery, API-key rotation, routing map and baseline export |
| Track B - website | Build Parent Stape in shadow, then repeat by franchise property | `Active guarded`; exact synthetic trigger/native GA4 tag exist only in unpublished workspace `3`, endpoint/client/FPID handling passed, and live browser routing remains off | Draft preview proof, browser/server parity, destination isolation, consent-state gating and business-delivery continuity pass |
