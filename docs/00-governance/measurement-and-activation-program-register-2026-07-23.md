...
| Parent website conversions | `Active guarded` | `school_inquiry_submit` still feeds the existing browser destinations. GA4, Meta CAPI `Inquiry Submit` and Google Ads server `Inquiry Submit_ollo` now run through Parent sGTM with the same Form `4` event ID used by the browser tags. The 2026-08-05 controlled saved Form `4` test passed transport and identity read-back; Events Manager pair deduplication remains an operational read-back | Monitor the first 48 hours, prove Meta browser/server merge quality in Events Manager and review Google conversion diagnostics after platform lag |
