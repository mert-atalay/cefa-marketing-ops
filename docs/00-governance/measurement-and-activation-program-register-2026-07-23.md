...
| Stape Business sGTM | `Active guarded` | Parent Stape Business runs in `CA East (Canada)` behind healthy first-party endpoint `edge.cefa.ca`. On 2026-08-04 server GTM published guarded version `2`, then web GTM published routing-only version `12`, `CEFA \| Parent \| GA4 via First-Party sGTM \| 2026-08-04`. The four-case no-preview matrix produced exactly two intended GA4 deliveries and blocked the wrong-host and wrong-event cases. A clean live homepage load sent one request to `edge.cefa.ca`, zero direct browser GA collection requests, and one Stape `PageView` delivery to GA4 with HTTP `204`. Google Ads, Meta, Gravity Forms, School Manager, KinderTales, CRM, and franchise routes were not changed | Monitor GA4 parity and the next legitimate `generate_lead`; build Google Ads and Meta server conversion routes only after transaction/event-ID deduplication, consent gating, and controlled conversion QA pass |
...
| Parent sGTM conversion-route expansion | CEFA measurement owner | Parent GA4 first-party production routing is active and verified; Google Ads and Meta remain on their unchanged browser paths | Add Google transaction identity and Meta `event_id` parity, destination consent gates, Meta CAPI credential, and controlled Form `4` deduplication proof before server conversion promotion |
