...
| Stape Business sGTM | `Active guarded` | On 2026-07-28 CEFA created Parent server GTM `GTM-T7C46VM7` and the matching Stape container in `CA East (Canada)`. Annual Business is paid and verified with `5M` requests/month, 10-day logs, 20-domain capability, and healthy billing. Parent web GTM `GTM-NZ6N7WNC` retains zero unpublished changes or conflicts and its live Parent GA4 Google tag has no `server_container_url` or transport override. In isolated server workspace `CEFA \| Parent \| sGTM Shadow Build \| 2026-07-28` (`3`), GTM MCP added exact-match trigger `CEFA \| Parent \| Shadow Health \| Event` for only `cefa_sgtm_shadow_health` and native server-GA4 tag `CEFA \| Parent \| GA4 \| Shadow Health`; read-back showed two pending changes, zero conflicts, no version and no publication. On 2026-07-29 the required Stape Canada CNAMEs, `Ready / Ok`, TLS and `/healthy` all passed. A PII-free `/g/collect` health hit returned HTTP `200` and the inherited GA4 client issued first-party `FPID`. Server Preview then accepted one PII-free `cefa_sgtm_shadow_health` request with HTTP `200`, fired the exact shadow tag once, and received HTTP `204` from GA4. Browser routing remains inactive, reported usage remains zero, and all optional power-ups remain off | Rotate the disclosed API key, complete account-control and rollback review, then approve the first controlled non-conversion browser-to-server shadow route before any publication or production routing |
...
| Parent sGTM first controlled browser route | CEFA measurement owner | Draft Preview passed on 2026-07-29, but no server version is published and the live Parent web GTM still has no transport override | After API-key rotation, account-control review, baseline export and rollback proof, approve a narrowly scoped non-conversion browser-to-server shadow route and reconcile browser/server delivery before conversion events |
...
| Track B - website | Build Parent Stape in shadow, then repeat by franchise property | `Active guarded`; endpoint/client/FPID and draft Preview passed, the exact synthetic tag fired once with GA4 HTTP `204`, and live browser routing remains off | Browser/server parity, destination isolation, consent-state gating and business-delivery continuity pass |
