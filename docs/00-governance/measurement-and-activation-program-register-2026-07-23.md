...
| Parent website conversions | `Active guarded` | `school_inquiry_submit` still feeds the existing browser destinations. GA4 routes through Parent sGTM, Meta CAPI `Inquiry Submit` is live with shared event identity, and Google Ads remains browser-only with the event ID as Transaction ID. Three inspected Meta server sends were accepted; Events Manager pair deduplication is not yet proven | Prove Meta browser/server deduplication in Events Manager; keep the Google server copy paused until one controlled saved Form `4` duplicate-counting test passes |
...
| Stape Business sGTM | `Active guarded` | Parent Stape Business runs in `CA East (Canada)` behind healthy `edge.cefa.ca`. Web version `15` preserves shared identity within GA4's 25-parameter limit. Server version `9` keeps guarded GA4 and Meta CAPI live; three inspected Meta sends were accepted. Google server tags remain paused. Hourly incoming `5xx`, outgoing `4xx` and outgoing `5xx` alerts plus one recipient are enabled | Verify Meta pair deduplication in Events Manager; run one controlled saved Form `4` Google duplicate-counting test before any Google server activation |
