...
| Parent website conversions | `Active guarded` | `school_inquiry_submit` still feeds the existing browser destinations. GA4 now routes through Parent sGTM, Meta CAPI `Inquiry Submit` is live with the browser event ID for deduplication, and Google Ads remains browser-only with the event ID as Transaction ID | Observe the next legitimate Meta inquiry and prove browser/server deduplication; keep the Google server copy paused until one controlled saved Form `4` duplicate-counting test passes |
...
| Dataform | `Active guarded` | Git now contains 18 assertion definitions. Local Dataform CLI compilation produced 18 actions with zero errors; the three new certified Parent inquiry assertions cover entry uniqueness, school completeness and event-to-daily reconciliation, and direct BigQuery proof queries pass. Cloud Run remains the production orchestrator | Run the expanded package through the CEFA-controlled Dataform release path, then establish dev/staging/prod releases |
| Stape Business sGTM | `Active guarded` | Parent Stape Business runs in `CA East (Canada)` behind healthy `edge.cefa.ca`. Web version `15` keeps `generate_lead` at the GA4 limit of 25 parameters while preserving shared identity and school context. Server version `9` keeps guarded GA4 live and activates Meta CAPI with exact `Inquiry Submit` name and browser event-ID deduplication; direct and sGTM Test Events passed. Google server conversion tags remain paused. No Gravity Forms, KinderTales, CRM, campaign goal or existing dashboard contract changed | Observe the next legitimate Meta inquiry and verify platform deduplication; run one controlled saved Form `4` Google duplicate-counting test before any Google server activation |
| Parent certified school/source reporting | `Active guarded` | Three additive non-PII BigQuery views now reconcile Gravity Form `4` inquiry and selected-school truth with exact event-ID GA4 session context. Through 2026-08-03, `3,689` source entries equal `3,689` event-view and daily-report inquiries; school completeness is 100% and exact GA4 eligibility is `3,331` (`90.3%`). The dashboard object is explicitly candidate-only | Connect only to a Looker test source, review visible identity/source exceptions, and obtain dashboard-owner approval before any serving-contract promotion |
...
| Stape recovery and secondary administrator evidence | CEFA | 2FA, API-key rotation, billing, auto-upgrade, analytics, outgoing logs, alert recipient and one monitoring rule are verified. Recovery-code custody and secondary administrator access are not visible through the API | Record CEFA-controlled recovery custody and backup administrator without exposing secrets |
| Parent sGTM conversion-route expansion | CEFA measurement owner | GA4 first-party routing and the 25-parameter payload guardrail are live. Meta CAPI is live under server version `9` after Test Events transport proof. Google browser Transaction ID is live and both Google server tags remain paused | Observe one legitimate Meta browser/server pair and confirm deduplication; run one controlled saved Form `4` Google duplicate-counting test before unpausing Google server delivery |
...
| Track B - website | Buy Parent Business and verify Stape/API/GTM control planes, domains and routing architecture | `Active guarded`; Business, Canada hosting, first-party endpoint, 2FA, durable API key, analytics, outgoing logs, alert recipient and one enabled 5xx rule are verified | Recovery custody and secondary administrator evidence |
| Track B - website | Build Parent Stape in shadow, then repeat by franchise property | `Active guarded`; Parent web version `15` and server version `9` are live. GA4 routing, payload limit, endpoint isolation, Meta transport and rollback points pass; Google server delivery remains paused | Observe a legitimate Meta pair, finish controlled Google deduplication QA, then independently build each franchise property |
| Track B - website | Promote Stape conversion routing one property at a time | `Partial`; Parent GA4 and Meta are live, Parent Google server copy is pending | Once-only delivery, platform deduplication and rollback test pass for each destination and property |
