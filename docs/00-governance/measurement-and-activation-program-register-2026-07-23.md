...
| Stape Business sGTM | `Active guarded` | Parent Stape Business runs in `CA East (Canada)` behind healthy first-party endpoint `edge.cefa.ca`. Server version `2` remains live. Web versions `13` and `14` added the existing stable event ID as the browser Google Ads Transaction ID and as `transaction_id` / `cefa_event_id` on the existing GA4 `generate_lead`; no destination, value, trigger, conversion action, campaign goal, or Meta tag changed. Unpublished server version `3` contains a strict Parent route, paused native Google tags, and the Stape Meta CAPI template. A post-build health check and PII-free non-conversion request returned HTTP `200` | Confirm the next legitimate `generate_lead` identity aliases, then run controlled Form `4` Google duplicate-counting QA; supply the Meta dataset token outside Git, create its paused server tag, and run Meta Test Events dedup QA before any server destination is published |
...
| Parent sGTM conversion-route expansion | CEFA measurement owner | Parent GA4 first-party routing, Google browser Transaction ID, shared `event_id` / `transaction_id` / `cefa_event_id` contract, strict server route, paused Google server tags, and Meta server template are complete; server delivery remains off | Verify the next legitimate event aliases, provide the Meta dataset token outside Git, create the paused Meta tag, then complete controlled Form `4` Google and Meta deduplication proof before publishing server version `3` or a successor |
...
| Track B - website | Build Parent Stape in shadow, then repeat by franchise property | `Active guarded`; Parent GA4 production route is active through server version `2` and web version `12`; allow/block, live endpoint, browser-route and outgoing GA4 tests pass | Monitor GA4 parity, then complete conversion-specific deduplication/consent QA and independently build each franchise property |
