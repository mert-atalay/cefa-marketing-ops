...
| Stape Business sGTM | `Active guarded` | On 2026-07-28 CEFA created Parent server GTM `GTM-T7C46VM7` and the matching Stape container in `CA East (Canada)`. Annual Business is paid and verified with `5M` requests/month, 10-day logs, 20-domain capability, and healthy billing. On 2026-07-29 CEFA enabled 2FA, rotated the disclosed full-account API key, revoked the old key, verified the replacement, enabled Stape Analytics and outgoing logs, retained one enabled alert recipient, and created enabled hourly monitoring for repeated outgoing `5xx` responses. Parent web GTM `GTM-NZ6N7WNC` retains zero unpublished changes or conflicts and its live Parent GA4 Google tag has no `server_container_url` or transport override. In isolated server workspace `CEFA \| Parent \| sGTM Shadow Build \| 2026-07-28` (`3`), exact-match trigger `CEFA \| Parent \| Shadow Health \| Event` accepts only `cefa_sgtm_shadow_health`, and native server-GA4 tag `CEFA \| Parent \| GA4 \| Shadow Health` is the only custom tag. Canada CNAMEs, `Ready / Ok`, TLS and `/healthy` passed. A current-token browser Preview test then returned HTTP `200`, matched only the PII-free health event, fired the intended GA4 tag, and received HTTP `204` from GA4; Stape outgoing logs independently recorded one `GA4` delivery with status `204`. Server workspace read-back remains two pending changes, zero conflicts, no version and no publication; live browser routing remains inactive and optional power-ups remain off | Export the rollback baseline, add consent-state and hostname/event guards, then run browser/server parity before any production routing or conversion-event promotion |
...
| Parent sGTM guarded production-route preparation | CEFA measurement owner | Current-token browser Preview and Stape outgoing-log evidence passed for the health-only event, but no server version is published and the live Parent web GTM still has no transport override | Export baseline/rollback evidence, add consent-state and hostname/event controls, then reconcile browser/server parity before conversion events |
...
| Track B - website | Buy Parent Business and verify Stape/API/GTM control planes, domains and routing architecture | `Active guarded`; Business, Canada hosting, first-party endpoints, 2FA, durable API key, analytics, outgoing logs, alert recipient and one enabled 5xx rule are verified | Recovery custody, secondary administrator and baseline/rollback export |
| Track B - website | Build Parent Stape in shadow, then repeat by franchise property | `Active guarded`; endpoint/client/FPID, current-token browser Preview and Stape outgoing-log evidence passed for the exact health-only event with GA4 HTTP `204`; live browser routing remains off | Browser/server parity, destination isolation, consent-state gating and business-delivery continuity pass |
