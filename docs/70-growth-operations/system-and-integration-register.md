...
| `SYS-STAPE` | Stape Business and direct API | Server-side GTM hosting plus account/container operations | Additive server route; direct API is the canonical machine interface and Stape MCP is supervised convenience only | `Active setup`; Parent annual Business verified, container running in Canada, `edge.cefa.ca` awaiting DNS, routing off |
