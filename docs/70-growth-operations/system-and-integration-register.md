...
| `SYS-GTM` | Web and server GTM containers | Property-specific event processing and destination mapping | Browser/server tag configuration; GTM API/MCP is the guarded automation interface | `Verified` web paths; Parent server foundation `GTM-T7C46VM7` created with no endpoint or production tags |
| `SYS-STAPE` | Stape Business and direct API | Server-side GTM hosting plus account/container operations | Additive server route; direct API is the canonical machine interface and Stape MCP is supervised convenience only | `Active setup`; Parent annual Business verified, container running in Canada, first-party endpoint `Ready / Ok` with valid TLS and health, routing off |
...
| `INT-D-005` | Websites/web GTM -> Stape-hosted server GTM -> destinations | First-party server transport and deduplication; direct Stape API governs hosting inventory while GTM API/MCP governs the container configuration | `Active setup`; Parent Business/control planes and first-party endpoint DNS/TLS/health verified. Parent web GTM baseline is clean and unchanged; separate server shadow workspace `3` is unpublished with one client and no tags, triggers or variables | Off | Browser loss remains; bad rollout could duplicate or cross destinations | [Cloud/Stape blueprint](../superpowers/plans/2026-07-25-google-cloud-stape-measurement-platform-blueprint.md) |
