...
| `SYS-GTM` | Web and planned server GTM containers | Property-specific event processing and destination mapping | Browser/server tag configuration; GTM API/MCP is the guarded automation interface | `Verified` web paths; server build pending |
| `SYS-STAPE` | Stape Business and direct API | Planned server-side GTM hosting plus account/container operations | Additive server route; direct API is the canonical machine interface and Stape MCP is supervised convenience only | `Approved`; Parent purchase/build pending |
...
| `INT-D-005` | Websites/web GTM -> Stape-hosted server GTM -> destinations | First-party server transport and deduplication; direct Stape API governs hosting inventory while GTM API/MCP governs the container configuration | `Approved`; Parent purchase/build pending | Off | Browser loss remains; bad rollout could duplicate or cross destinations | [Cloud/Stape blueprint](../superpowers/plans/2026-07-25-google-cloud-stape-measurement-platform-blueprint.md) |
